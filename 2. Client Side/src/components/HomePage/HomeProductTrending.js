          ...
          <div className="mt-6 grid grid-cols-2 gap-x-4 gap-y-10 sm:gap-x-6 md:grid-cols-4 md:gap-y-0 lg:gap-x-8">
            {products?.map((product) => (
              <Link
                ...
                <h3 className="mt-4 text-sm text-gray-700"><span className="absolute inset-0" />{product.name}</h3>
                <p className="mt-1 text-sm font-medium text-gray-900">${product.price}.00</p>
                <p className="mt-1 text-sm text-gray-500 text-justify">{product.description.slice(0, 200)}... <br /> <span className="text-blue-700 font-bold">{product.description.length > 200 ? "read more" : null}</span> </p>
              </Link>
            ))}
          </div>
